...
- `WIRE_VERSION` is `2` (since phase 4: kind-tagged bodies, P7). Bumping it is
...
|---|---|---|---|
| P1 | **Catch-up sync**: late joiners pull the signed offer/provider log from any member over the control ALPN and replay it through the standard verify path. Kills the "no gossip history" limitation. | new ALPN usage; gossip unchanged | ✅ phase 2 — `src/sync.rs`, `state.rs` retained frames, `tests/catch_up_sync.rs` |
| P2 | **Persistent identity** (builder-level; stable `EndpointId` across restarts — prerequisite for allowlists, reputation, "same peer" UX) | none | ✅ phase 2 — `StackOptions::identity_path` |
| P7 | **Kind-tagged bodies**: unknown message kinds are ignored *and relayed* instead of poisoning a frame. Wire version 2. | v2 frame layout | ✅ phase 4 — `BodyEnvelopeV1` |
| P8 | **Op-tagged control channel + `Hello`**: capability discovery, clean "unsupported operation" answers, new ops without version bumps. | additive on `/iroh-drop/1` | ✅ phase 4 |
| P9 | **Bounded state and flood control**: offer/provider/peer/alias caps with eviction, per-author quotas, per-peer token buckets, capped concurrent history serving. | none | ✅ phase 4 — `limits.rs`, `tests/hostile_peers.rs` |
| P10 | **Ordered self-asserted state**: `Provider.announced_at_ms` makes withdrawals survive stale relays. | additive field | ✅ phase 4 |
...
| # | Feature | Status |
|---|---|---|
| U1 | **`iroh-drop-sdk` crate**: config files, collections (recursive `add`, materialized `get`), inventory picking. Zero wire logic; depends only on `iroh-drop`'s public API. | ✅ phase 2 |
...
| U9 | **Short tickets** (now the default): name peers by id, let discovery find addresses. 116 chars vs 145–227, and immune to changing IPs. | ✅ phase 3 — `DropSession::short_ticket`, `tests/short_tickets.rs` |
| U10 | **Saved rooms**: `share --room team`, `receive --room team`, `rooms`. One ticket ever, then names forever; each join refreshes the room with a ticket that includes you. | ✅ phase 3 — `sdk::rooms` |
| U11 | **LAN drop discovery**: `share --lan` puts a short ticket in our mDNS record; `nearby` lists drops and `receive --nearby <#>` joins one. Opt-in per share, with an explicit broadcast warning. | ✅ phase 3 — `sdk::nearby` |
| U13 | **Embedding API**: `DropStack::from_parts` + `sync_handler()` so an app with its own endpoint/router/gossip/blobs can add drops. | ✅ phase 4 |
| U14 | **`OfferDecider` hook**: allowlists, prompts, per-project quotas — judgement in the app, limits in the protocol. | ✅ phase 4 |
| U15 | **`mdns` feature flag**: local discovery is opt-in at compile time, so embedders do not pay for multicast machinery. | ✅ phase 4 |
...
> Phase numbers are planning milestones, **not** release tags. App releases
> are tagged `v0.1.x` on GitHub; phases 2–4.x shipped as those releases.
> The wire format versions independently (`WIRE_VERSION = 2`).

- **Phase 2 (shipped)**: P1, P2, U1–U3. Protocol gained catch-up sync and
...
- **Phase 3 (shipped)**: onboarding — U7 (mDNS), U8 (QR), U9 (short tickets by
...
- **Phase 4 (shipped)**: extensibility and hardening — P7–P10, U13–U15, plus
...
- **Phase 4.x (shipped)**: U4 daemon + control API; `watch` UX; the macOS app
  ...
  stays theoretical.
- **Phase 4.y (shipped)**: the durability follow-through, after an external
  ...
  remains open.
- **Phase 4.z (shipped)**: sticky membership, everywhere. Joining a group is
...
- **Phase 5**: P3 swarm fetch (spike first); U5 MCP server.
- **Phase 6**: P5/P6 v2 family (private drops, tombstones); U6 TUI. Private
...
- No new gossip message variants: everything additive rode existing fields
  (`Offer.metadata`) or the reserved control ALPN, so the MVP (wire-v1) peers still
...
- **First-class extension subscriptions**: unknown kinds are relayed, but an
  ...
  use them. (Delivered: WS2 added kind subscriptions; WS9 replaced them with
  the namespaced `send_extension`/`on_extension` API — see `docs/extending.md`.)
